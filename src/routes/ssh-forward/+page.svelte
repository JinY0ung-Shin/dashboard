...
<div class="space-y-2">
	<div class="flex items-center justify-between">
		<div>
			<h2 class="text-lg font-semibold text-slate-200">SSH Tunnels</h2>
			<p class="text-slate-500 text-xs">
				{forwards.length} tunnels
			</p>
		</div>
		<div class="flex gap-1">
			<button on:click={toggleForm} class="glass-btn-secondary">
	...
	<div class="glass-card">
		...
		<div class="mt-2">
			...
			{:else if configEntries.length === 0}
				<div class="text-xs text-slate-500">No aliases found</div>
			{:else}
	...
	<div>
		...
		{:else if forwards.length === 0}
			<div class="text-center py-8 glass-card border-dashed border-slate-700">
				...
				<button on:click={toggleForm} class="glass-btn-primary"> Create Tunnel </button>
			</div>
		{:else}
