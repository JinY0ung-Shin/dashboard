...
<div class="glass-card border-l-2 border-l-blue-600">
	<h3 class="text-sm font-semibold text-white mb-2">Configure New Tunnel</h3>
	<form on:submit|preventDefault={onSubmit} class="space-y-3">
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-2">
			<div class="rounded border border-slate-800 bg-slate-900 p-2 space-y-2">
				...
				<div>
					<label for="name" class="text-xs text-slate-400 flex items-center gap-1">
						Name
				...
				<div>
					<label for="author" class="text-xs text-slate-400 flex items-center gap-1">
						등록자
						<Tooltip text={tooltips.author} />
					</label>
					<input
						id="author"
						type="text"
				...
				<div class="pt-2 border-t border-slate-700">
					<label class="flex items-center gap-2 cursor-pointer">
						<input
							type="checkbox"
							bind:checked={formData.litellmEnabled}
						...
						<span class="text-xs text-slate-300 flex items-center gap-1">
							LiteLLM 자동 등록
							<Tooltip text={tooltips.litellmEnabled} />
						</span>
					</label>
					{#if formData.litellmEnabled}
						<div class="mt-2 space-y-2">
							<div>
								...
								>
									모델 이름
									<Tooltip text={tooltips.litellmModelName} />
								</label>
								<input
									id="litellmModelName"
									type="text"
									bind:value={formData.litellmModelName}
									placeholder="my-llm"
									class="glass-input w-full text-xs"
								/>
							</div>
							<div>
								<label for="litellmApiKey" class="text-xs text-slate-400 flex items-center gap-1">
									API Key (선택)
									<Tooltip text={tooltips.litellmApiKey} />
								</label>
								<input
									id="litellmApiKey"
									type="password"
									bind:value={formData.litellmApiKey}
									placeholder="sk-..."
									class="glass-input w-full text-xs"
								/>
							</div>
			...
			<div class="rounded border border-slate-800 bg-slate-900 p-2 space-y-2">
				...
				<div class="grid grid-cols-2 gap-2">
					...
					<div>
						<label for="bindAddress" class="text-xs text-slate-400 flex items-center gap-1">
							Bind Address
							<Tooltip text={tooltips.bindAddress} />
						</label>
						<select
							id="bindAddress"
							bind:value={formData.localBindAddress}
							class="glass-input w-full"
						>
					...
					<div>
						<label for="remotePort" class="text-xs text-slate-400 flex items-center gap-1">
							Remote Port
							<Tooltip text={tooltips.remotePort} />
						</label>
						<input
							id="remotePort"
							type="number"
					...
					<div>
						<label for="remoteHost" class="text-xs text-slate-400 flex items-center gap-1">
							Remote Host
							<Tooltip text={tooltips.remoteHost} />
						</label>
						<input
							id="remoteHost"
							type="text"
			...
			<div class="rounded border border-slate-800 bg-slate-900 p-2 space-y-2">
				...
				<div>
					<label for="sshAlias" class="text-xs text-slate-400 flex items-center gap-1">
						SSH Alias
						<Tooltip text={tooltips.sshAlias} />
					</label>
					<select id="sshAlias" class="glass-input w-full" bind:value={selectedAlias}>
					...
					{#if selectedEntry?.proxyJump}
						<div class="text-[10px] text-violet-400 mt-1">
				...
				<div class="grid grid-cols-3 gap-2">
					<div class="col-span-2">
						<label for="sshHost" class="text-xs text-slate-400 flex items-center gap-1">
							SSH Host
							<Tooltip text={tooltips.sshHost} />
						</label>
						<input
							id="sshHost"
							type="text"
					...
					<div>
						<label for="sshPort" class="text-xs text-slate-400 flex items-center gap-1">
							Port
							<Tooltip text={tooltips.sshPort} />
						</label>
						<input
							id="sshPort"
							type="number"
		...
		<div class="flex gap-1 pt-2 border-t border-slate-800">
			<button type="submit" class="glass-btn-primary disabled:opacity-50" disabled={loading}>
				Start Tunnel
			</button>
